...
class BalanceChecker extends Component {
  ...
  async refreshData() {
    ...
    ethBalance = window.tronWeb.fromSun(
      window.tronWeb.BigNumber(ethBalance._hex).toString()
    ); //formatBigNumber(ethBalance, 2);
    //console.log("ethBalance", ethBalance ,  )
    // TODO: Tron: get exchange rate for Tron, not ETH
    const ethBalanceValue = ethBalance * ethRate;
    const balances = await Promise.all(
      availableSynths.map(synth => {
        /*
        console.info(`fetching balance for ${synth.name}`);
        console.log(synthetixJs[synth.name]);
        */
        return synthetixJs[synth.name].balanceOf(selectedWallet);
      })
    );
    /*
    const val = await synthetixJs.Synthetix.effectiveValue(
      getUtf8Bytes('sAUD'),
      balances[1],
      getUtf8Bytes('sUSD')
    );
    ...
    const totalBalance = (
      await Promise.all(
        balances.map(async (balance, i) => {
          try {
            const effectiveValue = await synthetixJs.Synthetix.effectiveValue(
            ...
            console.error(err);
            // TODO: @kev find out why this call fails with revert OPCODE
            // except for sUSD
            return null;
          }
        })
    ...
    return (
      <table cellPadding="0" cellSpacing="0" className={styles.table}>
        ...
        <tbody>
          ...
          <tr className={styles.tableBodyRow}>
            <td className={styles.tableBodySynth}>TRX</td>
            <td className={styles.tableBodyBalance}>
              <div>
  ...
  renderSynths() {
    ...
    return (
      <div>
        <h2 className={styles.tableHeading}>Asset Picker</h2>
        <SynthPicker />
